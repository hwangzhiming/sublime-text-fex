...
# __file__ is useless for my purposes. What I want is "Packages/Fex", but
...
def fex_root(directory):
    global fex_root_cache

    retval = False
    leaf_dir = directory

    if leaf_dir in fex_root_cache and fex_root_cache[leaf_dir]['expires'] > time.time():
        return fex_root_cache[leaf_dir]['retval']

    while directory:
        if os.path.exists(os.path.join(directory, 'fex.json')):
            retval = directory
            break
...
def fex_root_exist(directory):
    return fex_root(directory)
...
class CommandThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            if e.errno == 2:
                main_thread(sublime.error_message, "Fex binary could not be found in PATH\n\nConsider using the fex_command setting for the Fex plugin\n\nPATH is: %s" % os.environ['PATH'])
            else:
                raise e
...
class FexCmd(object):
    ...
    def run_command(self, command, callback=None, show_status=True,
        ...
        if 'working_dir' not in kwargs:
            kwargs['working_dir'] = self.get_fex_root()
        if 'fallback_encoding' not in kwargs and self.active_view() and self.active_view().settings().get('fallback_encoding'):
            kwargs['fallback_encoding'] = self.active_view().settings().get('fallback_encoding').rpartition('(')[2].rpartition(')')[0]

        s = sublime.load_settings("Fex.sublime-settings")
        # if s.get('save_first') and self.active_view() and self.active_view().is_dirty() and not no_save:
        #    self.active_view().run_command('save')
        # if command[0] == 'fex' and s.get('fex_command'):
        #    command[0] = s.get('fex_command')
        # if command[0] == 'fex-flow' and s.get('fex_flow_command'):
        #    command[0] = s.get('fex_flow_command')
        if not callback:
    ...
    def generic_done(self, result):
        ...
        if view and view.settings().get('live_fex_annotations'):
            self.view.run_command('fex_annotate')
    ...
    def panel(self, output, **kwargs):
        if not hasattr(self, 'output_view'):
            self.output_view = self.get_window().get_output_panel("fex")
        self.output_view.set_read_only(False)
        self._output_to_view(self.output_view, output, clear=True, **kwargs)
        self.output_view.set_read_only(True)
        self.get_window().run_command("show_panel", {"panel": "output.fex"})
...
class FexWindowCommand(FexCmd, sublime_plugin.WindowCommand):
    ...
    # that the user intends Fex commands to run against when there's only
    # only one.
    def is_enabled(self):
        if self._active_file_name() or len(self.window.folders()) == 1:
            return fex_root(self.get_working_dir())
    ...
    # search for the Fex root.  Otherwise, use the only folder that is
    # open.
...
class FexTextCommand(FexCmd, sublime_plugin.TextCommand):
    ...
    def get_relative_file_name(self):
        ...
        file_path = working_dir.replace(fex_root(working_dir), '')[1:]
...
class FexCustomCommand(FexWindowCommand):
    may_change_files = True

    def run(self, command=None):
        if command is None:
            self.get_window().show_input_panel("Fex command", "",
